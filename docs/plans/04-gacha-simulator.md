...
상태: 완료 — 백엔드 구현 + 웹 실 API 전환 및 e2e 검증 완료 (2026-07-15). 모바일(Flutter)은 보관, 앱은 웹 `SimulatorPage`를 셸로 사용
...
- 구현 메모 (2026-07-15):
  - 백엔드 API가 아직 없어 기본값은 Mock(`lib/repositories/mock_gacha_repository.dart`)으로 동작. 백엔드 완성 후 `--dart-define=USE_MOCK_GACHA=false`로 실 API 전환 (`lib/config/app_config.dart`)
  - API 베이스 URL은 `--dart-define=API_BASE_URL=...`로 지정 (기본 `http://localhost:8080`, Android 에뮬레이터는 `http://10.0.2.2:8080`)
  - 비로그인 천장 카운트는 배너별로 `shared_preferences`에 보관(`gacha_pity_{bannerId}`), 로그인 연동(파트 02) 후 서버 카운트 우선으로 교체 예정
  - `POST /api/banners/{bannerId}/pull` 요청 본문을 `{"count": 1|10, "currentPity": n}`, 응답을 `{"results": [Character...], "pityCount": n}` 형태로 가정 — 백엔드 구현 시 이 계약에 맞추거나 모델(`lib/models/`) 수정 필요
...
- [x] 웹에서 동일 API로 동작 확인 — H2 인메모리로 실서버 기동 후 웹이 쓰는 전 엔드포인트 e2e 통과(계약 필드/천장/리셋/확정, SYNC.md frontend #4 로그 참고). 모바일은 전략 변경으로 Flutter 보관 — 앱(mobile_shell)은 웹 SimulatorPage를 그대로 사용하므로 별도 확인 불필요
